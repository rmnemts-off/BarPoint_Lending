...
window.BARPOINT_CASES = [
  ...
  },
  {
    ...
    /* Кадры 05.08 из архива заказчика «Часть 1»: «Интерьер» — на внешнюю
       карточку (cardImage), нумерованные — в разворот, порядок по номеру.
       Правка 06.08 (слова заказчика): кадры 1 и 2 поменяны местами —
       разворот открывает чайник с чашками и десертами (файл -2-), меню
       «Зойкин газетъ» (файл -1-) идёт вторым. Номера в именах файлов
       остались от порядка в архиве, файлы не переименовывались. */
    cardImage: "case-zoyka-cover-1200",
    gallery: ["case-zoyka-2-1100", "case-zoyka-1-1100", "case-zoyka-3-1100", "case-zoyka-4-1100"]
  },
  {
    ...
    /* Кадры 06.08 (третий заход, присланы в чате): «Интерьер» со STREAM
       ZONE — на внешнюю карточку, два коктейля — в разворот, порядок как
       прислан. Розовый неон взят в улучшенной версии 1265×1244 вместо
       исходных 900×885 (композиция та же, деталей больше), белый —
       наоборот, исходником 2000×1333: он крупнее «улучшенного» 1537×1023.
       Прежние кадры (case-memo-1-900, -2-1400, -2-800) удалены. */
    cardImage: "case-memo-cover-1125",
    gallery: ["case-memo-1-1265", "case-memo-2-1890"]
  },
  {
    ...
    /* Кадры 06.08 (третий заход, присланы в чате): интерьер с кессонным
       потолком — на внешнюю карточку, три коктейля с фирменной вывеской
       BURO. TSUM — в разворот, порядок как прислан. Прежние три кадра
       (case-buro-1-1200, -2-1200, -3-1120) и их дубли -800 удалены.
       Кадры горизонтальные (3:2), а коробка разворота вертикальная —
       cover срезает по 42% ширины с боков; и бокал, и вывеска стоят по
       центру кадра, поэтому срез их не задевает. */
    cardImage: "case-buro-cover-1200",
    gallery: ["case-buro-1-1537", "case-buro-2-1537", "case-buro-3-1536"]
  },
  {
  ...
  },
  {
    ...
    /* Кадры 06.08 (третий заход, присланы в чате): интерьер лофта с
       кирпичом — на внешнюю карточку, четыре коктейля — в разворот,
       порядок как прислан. Прежние два кадра (case-duran-2-1000,
       case-duran-1-620) удалены.
       У третьего кадра (флейта с бантом) кадр дорощен по бокам — см.
       общий комментарий о полях у PIMS ниже; фон там тёмное боке, и
       зеркальное продолжение в нём не читается.
       ОГОВОРКА ПО КАЧЕСТВУ: исходники 1, 2, 3 и 4 маленькие (682×1024,
       914×1024 и 1000×667). Коробка разворота на retina просит 1467×1680,
       то есть эти кадры дают 40–61% нужного разрешения и на большом экране
       будут мылить. Если у заказчика есть оригиналы крупнее —
       переэкспортировать.
       Правка 06.08: снимок с черникой (case-duran-1-1100) заменён по просьбе
       заказчика на коктейль в бокале-черепе. Качество webp 85, а не 82 как
       у соседей: кадр тёмный, две трети площади — плавные градиенты боке, а
       по ним webp первым идёт полосами. Замер отклонения от исходника: по
       всему кадру среднее 1.77 при максимуме 37, по тёмным зонам — 1.15 при
       максимуме 18, то есть полос нет.

       Правка 06.08 (вторая): порядок кадров задан заказчиком поштучно —
       флейта с бантом, череп, медная кружка, серебряный бокал. Имена
       файлов остались от порядка в архиве, файлы не переименовывались.

       Тогда же «череп» и «серебряный бокал» отдалены. Резало их не
       кадрирование файла, а object-fit: cover: коробка галереи на десктопе
       считается как ((100vw−30)/2)/(100vh−20), это 0.77…0.89, а пропорция
       обоих кадров была 0.666 — cover срезал 16–22% ВЫСОТЫ. Крупнее
       исходника кадр не сделать, поэтому пропорция файла подведена к
       пропорции коробки: 0.666 → 0.799, снимок внутри стал меньше, обрезка
       почти ушла. Поле по 68px с каждой стороны достроено не заливкой, а
       продолжением самой фотографии — тем же приёмом, что у первого кадра
       PIMS: кромка отражается, и размытие НАРАСТАЕТ ОТ ШВА. У самого шва
       его нет вовсе (отражение там совпадает с кадром пиксель в пиксель),
       первые 12% поля не тронуты, дальше идёт разгон до максимума у
       внешнего края. Первая попытка размывала полосу равномерно, и на шве
       выходил скачок резкости — заказчик прочёл его как обрезку.
       Профиль сверен по PIMS (резкость на шве 97% от кадра, спад к краю
       втрое). Замер на месте, шагом 8px: у «черепа» 1.97 снаружи шва
       против 1.97 внутри и 0.38 у края, у «кубка» 3.48 против 3.58 и 1.17
       — ступени на шве нет ни там, ни там.
       Инструмент: scratchpad/duran_feather.py. */
    cardImage: "case-duran-cover-1200",
    gallery: ["case-duran-3-914", "case-duran-1-819", "case-duran-4-1000", "case-duran-2-819"]
  },
  {
  ...
  },
  {
    ...
    /* Правка 06.08: у четвёртого кадра (бокал с глазом) уменьшен масштаб —
       см. общий комментарий о полях у PIMS. Бокал во весь рост выше окна
       разворота, и центр-кроп срезал ему ножку. */
    gallery: ["case-luwo-1-1100", "case-luwo-2-1100", "case-luwo-3-1100", "case-luwo-4-1440"]
  },
  {
  ...
  },
  {
    ...
    /* ПРАВКА 06.08 — «УМЕНЬШИТЬ МАСШТАБ» У ПЕРВОГО КАДРА (просьба клиента:
       «обрезается напиток, и это очень некрасиво»).
       Коробка разворота вертикальная, но её пропорция плавает от высоты
       окна: 0.842 на 1512×900 и 0.943 у клиента (окно ниже — панель
       закладок и док). Кадр был 1100×1971, то есть вытянут ещё сильнее
       (0.558), поэтому object-fit: cover тянул его по ШИРИНЕ и срезал по
       высоте 34–41%. Сам стакан (1567px) выше окна (1165px на 0.943), так
       что сдвигом кропа его не спасти — лечится только уменьшением
       масштаба, а для cover это значит: сделать кадр ШИРЕ при той же
       высоте. Кадр дорощен по бокам на 13% зеркальным отражением кромки,
       поля уведены в расфокус — у шва размытия нет вовсе (отражение там
       совпадает с кромкой пиксель в пиксель), к внешнему краю нарастает.
       Выходит не пустая полоса, а продолжение фона с малой глубиной
       резкости; у PIMS фон и так гладкий градиент, шва не видно.
       ГДЕ ПРИЁМ НЕ ГОДИТСЯ: если предмет упирается в кромку кадра,
       зеркало даёт видимого двойника (проверено на смузи и кубке Duran
       Bar — там поля не ставились). Нужен мягкий фон у краёв.
       Инструмент: scratchpad/padfit.py. */
    gallery: ["case-pims-1-1386", "case-pims-2-674", "case-pims-3-2257", "case-pims-4-857"]
  },
  {
