...
(function () {
  ...
  /* ---------- карточки услуг ----------
     ЛЕНТА ПЕРЕНЕСЕНА ИЗ ИСХОДНИКА РЕФЕРЕНСА, А НЕ ВОССТАНОВЛЕНА ПО КАДРАМ.
     05.08 в чанке 600d175e (модуль 83386) buckssauce найден сам обработчик
     карточки, а в чанке 812e30c7 (модуль 46607) — его токены движения.
     Дословно оттуда:

       gsap.set(q,  {opacity:0, y:80, rotation:"random(-20,20)", scale:.6});
       gsap.set(bg, {opacity:0, scale:.8});
       observer(row, () => {
         const tl = gsap.timeline();
         tl.to(q,  {opacity:1, y:0, rotation:i%2==0?-3:3, scale:1,
                    duration:.8, ease:easings.back.medium}, 0);
         tl.to(bg, {opacity:1, scale:1, duration:.7, ease:"power4.out"}, 0);
         anims.title(title, tl, ">-0.35");
         tl.set(items, {opacity:0, yPercent:50}, ">-0.3");
         tl.to(items, {opacity:1, yPercent:0, duration:.8,
                       stagger:.12, ease:easings.copy}, "<");
       }, {threshold:.15});

     а anims.title — это SplitText на «lines,words,chars» и знаки из
     {opacity:0, y:60, scaleX:.8, scaleY:.5} за durations.title (.48) с
     шагом staggers.title (.028) на back.out(1.7).

     ЧТО ЭТО ИСПРАВИЛО. Прежняя версия была снята покадрово и разошлась с
     оригиналом в двух местах. Первое: заголовок панели проявлялся ТОЛЬКО
     прозрачностью, без движения и масштаба, — замер тогда не увидел ни
     сдвига, ни сжатия знаков, хотя в исходнике они есть. Второе: пункты
     шли .68с шагом .117 вместо .8 и .12.

     СМЕЩЕНИЯ СНОВА ОТНОСИТЕЛЬНЫЕ, и это не риск, а точность. «>» в GSAP —
     конец ПРЕДЫДУЩЕЙ анимации, а не всей ленты, поэтому ">-0.35" считается
     от панели (.7с) и даёт ровно 0.35с; прежнее замеренное число было .37.
     Список стартует за .3с до конца заголовка, то есть зависит от числа
     знаков: 0.35 + .48 + .028·(n−1) − .3. Для референсной карточки в 18
     знаков это 1.006 — против 1.02, снятых с экрана. Обе метки сошлись с
     замером до сотых, так что жёсткие числа больше не нужны, а заголовки
     разной длины теперь ведут за собой список ровно так же, как там. */
  function playCard(row, i) {
    ...
    /* Заголовок панели. Шаг между знаками — ровный S.title, БЕЗ нашего
       titleStagger: тот ужимает каскад на длинных строках, а заголовки
       карточек и здесь, и у референса одной длины (18–22 знака против
       18–21), сжимать нечего. Центр вращения — умолчание GSAP (50% 50%),
       как в оригинале; общий titleTl проекта тянет знаки от нижней кромки,
       и это его собственная особенность, не референсная. */
    if (title) {
      var tsp = splitOf(title, "lines,words,chars");
      tl.set(tsp.chars, { opacity: 0, y: 60, scaleX: .8, scaleY: .5 }, ">-0.35")
        .set(title, { opacity: 1 }, "<")
        .to(tsp.chars, {
          opacity: 1, y: 0, scaleX: 1, scaleY: 1,
          duration: D.title, stagger: S.title, ease: E.back.base,
          onComplete: finish(title, tsp)
        }, "<");
    }
    /* Пункты списка: opacity 0→1 и y ровно на половину своей высоты
       (yPercent 50), ни масштаба, ни поворота. */
    if (items.length) {
      tl.set(items, { opacity: 0, yPercent: 50 }, ">-0.3")
        .to(items, {
